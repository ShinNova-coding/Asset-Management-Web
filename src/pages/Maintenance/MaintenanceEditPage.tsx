...
import { ArrowLeft, CheckCircle, ImageIcon, Wrench, X } from "lucide-react"
...
export default function MaintenanceEditPage() {
  ...
  useEffect(() => {
    ...
    setCompletedDate(record.completed_date || today)
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!completedDate.trim()) {
      showToast("Completed Date is required.")
      return
    }
        ...
        <form onSubmit={handleSubmit} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="grid gap-5 p-6 md:grid-cols-2">
            ...
            <label className="grid gap-1.5">
              ...
              <input type="date" value={record.maintenance_date || ""} readOnly className={inputCls} />
            </label>

            <label className="grid gap-1.5">
              <span className="text-xs font-semibold uppercase tracking-wide text-slate-600">Completed Date</span>
              <input type="date" value={completedDate} onChange={(event) => setCompletedDate(event.target.value)} className={`${inputCls} !bg-white`} />
            </label>
