...
const AddNewAsset = () => {
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      ...
      try {
        const usersResponse = await fetch("http://192.168.100.183:1010/api/user", { headers });
        if (usersResponse.ok) {
          const usersData = await usersResponse.json();
          setUsersList(usersData.data?.data || usersData.data || []);
        }

        const assetsResponse = await fetch("http://192.168.100.183:1010/api/asset", { headers });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const API_URL = "http://192.168.100.183:1010/api/assignment"; 
